...
# commented out, cannot run on a single gene
...
dct <- read.table('/mnt/cold1/snaketree/prj/hn/local/share/data/gli2_dct.txt', sep="\t", header=T) # 30 HNC

dctpdx <- dct[grepl('HNC', dct$model), ]

ggplot(data=dctpdx, aes(x=label, y=dct, fill=label))+geom_boxplot(outlier.shape=NA)+geom_jitter(height=0)

#
dctpdx[dctpdx$model=="HNC0070", 'label'] <- 'SENSITIVE'
ggplot(data=dctpdx, aes(x=label, y=dct, fill=label))+geom_boxplot(outlier.shape=NA)+geom_jitter(height=0)


rnas <- data.frame(model=substr(names(nuovo_vsd), 0,7), GLI2=unlist(nuovo_vsd[1,, drop=T])) # 42

m <- merge(dctpdx, rnas, by="model")

ggplot(data=m, aes(x=dct, y=GLI2))+geom_smooth(method='lm')+geom_point() # 13

cor.test(m$dct, m$GLI2)

ggplot(data=m, aes(x=label, y=dct, fill=label))+geom_boxplot(outlier.shape=NA)+geom_jitter(height=0)
ggplot(data=m, aes(x=label, y=GLI2, fill=label))+geom_boxplot(outlier.shape=NA)+geom_jitter(height=0)

dfresp <- read.table('/mnt/cold1/snaketree/prj/hn/local/share/data/SampleTablenoCRLF.txt', sep="\t", header=T)
df <- unique(data.frame(model=substr(dfresp$SampleID, 0,7), cet=dfresp$Cetuximab_Response))

ggplot(data=m2, aes(x=cet, y=dct, fill=cet))+geom_boxplot(outlier.shape=NA)+geom_jitter(height=0)


m2 <- merge(dctpdx, df, by="model")
##############
...
### GLI2 vs anatomical location TODO