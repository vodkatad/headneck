...
geni <- "/mnt/cold1/snaketree/prj/hn/dataset/V1/WES/TCGA_sign.tsv"
geni <- read.table(geni, quote = "", sep = "\t", header = FALSE, stringsAsFactors = FALSE)
geni <- geni$V1
...
xeno <- as.data.frame(t(xeno))
xeno$sum <- rowSums(xeno)
xeno <- xeno %>% filter(sum > 1)
sum <- xeno
sum$sum <- NULL
sum <- as.data.frame(t(sum))
sum$sum <- rowSums(sum)
sum <- sum[order(sum$sum),]
sum$gen <- rownames(sum)
xeno$sum <- NULL
xeno <- as.data.frame(t(xeno))
...
percentage <- as.data.frame(t(xeno))
percentage$total_samples <- ncol(percentage)
percentage$mutation_count <- rowSums(percentage == 1)
percentage$mutation_pct <- (percentage$mutation_count / percentage$total_samples) * 100
rownames(percentage) <- paste0(rownames(percentage), "_", percentage$mutation_pct, "%")
percentage$mutation_count <- NULL
percentage$total_samples <- NULL
percentage$mutation_pct <- NULL

mat_list <- list(HR=as.matrix(percentage))
...
op <- oncoPrint(ordered_mat_list, alter_fun = alter_fun, col = col, row_order = names(su),
                remove_empty_columns = FALSE, remove_empty_rows = FALSE, column_names_gp=gpar(fontsize=8), show_pct=FALSE)
...
pdf("/mnt/cold1/snaketree/prj/hn/dataset/V1/WES/oncoprint_TCGA.pdf")
op
dev.off()
